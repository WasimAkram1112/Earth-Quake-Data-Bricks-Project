...
@dlt.view(name="data_flattening_vw")
def flatten_data():
    df = (
        spark.readStream.format("cloudfiles")
        .option("cloudFiles.format", "json")
        .load(volumne_path)
        .withColumn("_load_timestamp", current_timestamp()) # For SCD - 2
    )
    df = df.withColumn("parsed_data", from_json(col("features"), schema))
    # To flatten the features key using explode function of json file
...
dlt.create_streaming_table(name="final_data")

dlt.apply_changes(
    target="final_data",
    source="data_flattening_vw",
    keys=[primary_key],
